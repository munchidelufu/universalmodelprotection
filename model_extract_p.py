...
def train(
    ...
    for _, batch_data in enumerate(data_loader):
        ...
        loss = loss_fn(
            F.softmax(s_output / args.T, dim=1),
            F.softmax(t_output / args.T, dim=1),
        ) * (args.alpha) + (1 - args.alpha) * F.cross_entropy(s_output, pred)
        #
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
...
def fit(
    ...
    #
    save_dir = f"./bci_model/model_extract_p/{args.teacher_name}_{args.student_name}/model_extract_{index}"
    ...
    for split_idx, (train_dataset, test_dataset) in enumerate(kfold.split(dataset)):
        ...
        for epoch_id in range(args.epochs):
            ...
            test_loss, test_acc = utils.test(
                model=stu_model, data_loader=test_loader, loss_fn=val_loss_fn
            )
            writer.add_scalar(
        ...
        loss, test_acc = utils.test(stu_model, test_loader, loss_fn=val_loss_fn)
...
if __name__ == "__main__":
    ...
    # teacher dataset
    _, attack_dataset = bci_loader.load_split_dataset(args.teacher_name)
    # pair_kfold
    kfold = KFoldGroupbyTrial(
        n_splits=5,
        split_path=f"./bci_data/{args.teacher_name}/attack",
    )
